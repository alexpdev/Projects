...
function humanizeBytes(amount) {
  ...
  if (1024 <= amount < 1048576) return amount / 1024 + " KiB";
  if (1048576 <= amount < 1073741824) return amount / 1048567 + " MiB";
  return amount / 1073741824 + " GiB";
}

function normalizePieceLength(pieceLength) {
  if (pieceLength instanceof String) pieceLength = parseInt(pieceLength);
  if (13 < pieceLength <= 27) return Math.pow(2, pieceLength);
  if (pieceLength > 28) {
...
function fileListTotal(path) {
  ...
  if (stats.isFile()) return { path: [path], size: stats.size };
  let total = 0;
  let fileList = [];
